...
Tally has been in daily real use since **v1.0.0** and is currently at **v1.12.0**. The MVP and the whole V2 scope in [`PRD.md`](./PRD.md) are delivered; development continues in releases. The implementation was built in vertical slices — each one a thin end-to-end feature with UI, logic, persistence, and tests.
...
- [x] **v1.11.1** (ago/2026) — backup restore hardening. The handler took a path from the renderer and `copyFileSync`'d it over the database, checking only that the file existed — a confused deputy. Severity was low (the renderer has no injection surface), but the guard is cheap and also catches a renderer bug sending the wrong path. The invariant is now "you can only restore what the app itself lists". Wiring it up surfaced a pre-existing bug: the safety backup's retention could delete the very copy being restored
- [x] **v1.11.2** (ago/2026) — statement lifecycle and boundary guards. The SQLite wrapper never finalized prepared statements: 20k prepares cost ~44 MB of RSS, and worse, `Statement.get()` leaves its cursor open, so the table refuses DDL — the measured root cause of a "database table is locked" that had been documented in the migration tests as if it were their own limitation. Production only escaped by accident of ordering. Also: the `will-navigate` guard now requires the same document instead of prefix-matching strings, the hidden PDF window got the guards the main window already had, the `electron/` layer got its first tests (the vitest `include` could not even reach it), and the `npm audit` gate went green once two unused-in-practice dependencies stopped dragging `electron` into the production tree
- [x] **v1.11.3** (set/2026) — three summaries stuck on "today" next to a control that changes the month. The Faturas card track always shows each card's _current_ statement, by design (RF-FAT-06), but nothing said so — the card doubles as the panel's selector, and selection implies identity, so a panel navigated elsewhere read as a bug. Cards now name the statement they're showing and flag it when the focused card diverges from the panel. Same sweep found the Cartões row still labeling that value "Fatura aberta" after it closed or got paid — a label bug, not a calculation one — and the monthly agenda calling a future month's whole span "próximos N dias" counted from today, which in September made December read as "próximos 31 dias"
- [x] **v1.11.4** (set/2026) — five defects from an open-ended code sweep, none of them reported from use. The hourly maintenance timer captured the `Database` in its closure, so `Criar backup agora` and `Restaurar backup` — which close and reopen SQLite around the file copy — left it pointing at a closed connection: RN-06 auto-close and the OS notifications both died for the rest of the session, silently, because the error went to a console a packaged binary doesn't have. Advancing an installment moved `fatura_id` without `data_referencia`, undoing migration 0010's backfill row by row and making the February CSV export carry June and July dates (migration 0012 repairs it). Unarchiving an income source that never had a received entry left it active in the list and generating nothing, forever. Extending a subscription's horizon re-applied RN-01 to an already-computed month, which on a card closing on day 1 skipped a month permanently. And the safety backup taken right before a data import ignored the configured backup folder and retention
- [x] **v1.12.0** (set/2026) — the first new feature since the V2 scope closed: a **Simulation** screen that answers "what if I spend X?" without registering anything. Until now the only way to test a hypothesis was to create a fake expense and delete it, which touches statements, installments and the RN-06 lifecycle for a question that isn't about real data. The idea arrived named "Forecast" and was renamed before any code: in Tally, _forecast_ and _projected_ already mean **real data that hasn't happened yet** (an Expected receipt, an Open statement), and reusing the words would imply the number feeds RN-08 — it feeds nothing. Each hypothesis carries a repeat multiplier and an on/off toggle that removes it from the total without removing it from the list. The starting point is either the month's projected balance or **a value you type**, which exists because the model tracks monthly flow and not account balance: "I have 200 in the bank today" isn't derivable from anything the app stores. It persists to its own JSON file rather than sharing `settings.json`, whose schema invalidates the whole file on one bad field — a corrupted simulation would take the theme, the backup folder and the retention with it. The `electron/ipc/` layer got its first tests along the way
